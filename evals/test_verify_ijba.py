...
def load_feat(split_img_dir, img_type, list_file, bin_file):
	# img_name_file = []
	# with open(list_file, 'r') as imf:
	# 	for line in imf:
	# 		img_name = line.strip().split('/')[-1]
	# 		img_name_file.append(os.path.join(split_img_dir, img_type, img_name))
	mid_feats = dict()
...
def test_verify():
	...
	for split in range(1, split_num + 1):
		...
		for img_type in ['frame', 'img']:
			...
			bin_file = os.path.join(split_img_dir, '{}_feat.bin'.format(img_type))
			mid_feats, feat_dim = load_feat(split_img_dir, img_type, list_file, bin_file)
