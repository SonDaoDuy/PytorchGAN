...
def test_recog(save_dir):
	...
	for split in range(1, split_num + 1):
		...
		for image_type in ['gallery', 'probe']:
			list_file = os.path.join(split_img_dir, '{}_list.txt'.format(image_type))
			bin_file = os.path.join(split_img_dir, '{}_feat.bin'.format(image_type))
